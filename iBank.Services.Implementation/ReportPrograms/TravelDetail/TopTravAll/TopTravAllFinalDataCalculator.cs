...
namespace iBank.Services.Implementation.ReportPrograms.TravelDetail
{
    public class TopTravAllFinalDataCalculator
    {
        ...
        public List<TopTravAllFinalData> GetFinalDataFromRawData(List<TravDetRawData> rawDataList, List<CarRawData> cars, List<HotelRawData> hotels,
            IQuery<IList<MasterAccountInformation>> getAllMasterAccountsQuery, ReportGlobals globals, bool homeCountry, bool displayCo2, bool hasRouting)
        {
            ...
            var results = new List<TopTravAllFinalData>();
            ...
            foreach (var reckey in reckeys)
            {
                ...
                var finalData = new TopTravAllFinalData { RecKey = reckey };
                if (tripRec != null)
                {
        ...
        {
            if (includeCarbonEmissions && homeCountry)
                return "ibTopTravAllCarb2";
            else if (includeCarbonEmissions && !homeCountry)
                return "ibTopTravAllCarb";

            return homeCountry ? "ibTopTravAll2" : "ibTopTravAll";
        }
        ...
        private TopTravAllFinalData AddHotelRecords(TopTravAllFinalData finalData, TravDetRawData tripRec, List<HotelRawData> hotelRecs, bool carRecs, bool homeCountry)
        {
        ...
        private TopTravAllFinalData AddCarRecords(TopTravAllFinalData finalData, TravDetRawData tripRec, List<CarRawData> carRecs, bool homeCountry)
        {
            if (tripRec == null)
            {
        ...
        private TopTravAllFinalData AddTripRecord(TopTravAllFinalData finalData, TravDetRawData tripRec, bool homeCountry, bool displayCo2)
        {
