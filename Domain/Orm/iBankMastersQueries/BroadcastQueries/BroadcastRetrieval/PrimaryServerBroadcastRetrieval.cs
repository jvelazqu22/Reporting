...
{
    public class PrimaryServerBroadcastRetrieval : IBroadcastRetrieval
    {
        ...
        public PrimaryServerBroadcastRetrieval(IMasterDataStore store, ICacheService cache,
        ...
        public List<bcstque4> GetBroadcasts(IQueryable<bcstque4> queue)
        {
            ...
            LOG.Debug($"[{BroadcastServerFunction.Primary}] - Retrieved [{recs.Count}] broadcasts prior to filtering out long running broadcasts.");

            var svc = new LongRunningService(_cache, _store);
            var longRunning = recs.Where(x => svc.IsLongRunningBroadcast(x)).ToList();
            LOG.Debug($"[{BroadcastServerFunction.Primary}] - [{longRunning.Count}] of [{recs.Count}] are considered long running.");
